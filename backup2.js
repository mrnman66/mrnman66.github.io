(function() {
  ...
  function goImport() {
    ...
    input.onchange = function(event) {
      ...
      if (!file) {
        Lampa.Noty.show(Lampa.Lang.translate('lampac_backup_import_fail'));
        Lampa.Controller.toggle('settings');
        return;
      }
      ...
      reader.onload = function(event) {
        try {
          ...
          let keys = Object.keys(importedSettings);
          ...
          // Уведомление об успехе и перезагрузка через 5 сек
          Lampa.Noty.show(
            Lampa.Lang.translate('lampac_backup_import_success') +
            ' - ' + Lampa.Lang.translate('lampac_backup_imported') +
            ' (' + keys.length + ') - ' + Lampa.Lang.translate('lampac_backup_reload_after')
          );
          setTimeout(function() {
            window.location.reload();
          }, 5000);
        } catch (error) {
          Lampa.Noty.show(Lampa.Lang.translate('lampac_backup_import_fail') + ': ' + error.message);
        }
        Lampa.Controller.toggle('settings');
      };
  ...
  function startPlugin() {
    ...
    // Добавляем переводы (только нужные)
    Lampa.Lang.add({
      lampac_backup: {
        ru: 'Бэкап',
        en: 'Backup',
      ...
      lampac_backup_import_success: {
        ru: 'Бэкап успешно восстановлен',
        en: 'Backup successfully restored',
        uk: 'Резервна копія успішно відновлена',
        zh: '备份成功恢复'
      },
      lampac_backup_imported: {
        ru: 'Импортировано',
        en: 'Imported',
        uk: 'Імпортовано',
        zh: '已导入'
      },
      lampac_backup_reload_after: {
        ru: 'Перезагрузка через 5 секунд',
        en: 'Reloading in 5 seconds',
        uk: 'Перезавантаження через 5 секунд',
        zh: '5秒后重新加载'
      },
      lampac_backup_import_fail: {
        ru: 'Ошибка восстановления бэкапа',
        en: 'Backup restoration failed',
        uk: 'Помилка відновлення резервної копії',
        zh: '备份恢复失败'
      }
    });
