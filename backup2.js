(function() {
  ...
  function restoreSettingsFromFile(file) {
    ...
    reader.onload = function(event) {
      try {
        ...
        if (importedSettings.storage && typeof importedSettings.storage === 'object') {
          Object.keys(importedSettings.storage).forEach(key => {
            localStorage.setItem(key, importedSettings.storage[key]);
          });
        }
  ...
  function startPlugin() {
    window.lampac_app_settings_plugin = true;

    Lampa.Lang.add({
      lampac_app_settings: {
        ru: 'Бэкап',
        en: 'App Settings',
        uk: 'Налаштування додатка',
        zh: '应用设置'
      },
      lampac_app_settings_save: {
        ru: 'Сохранить',
        en: 'Save Settings',
        uk: 'Зберегти налаштування',
        zh: '保存设置'
      },
      lampac_app_settings_load: {
        ru: 'Восстановить',
        en: 'Load Settings',
    ...
    Lampa.SettingsApi.addParam({
      ...
      field: {
        name: Lampa.Lang.translate('lampac_app_settings_save'),
      },
      onChange: function() {
        saveSettingsToFile();
      }
    ...
    Lampa.SettingsApi.addParam({
      ...
      field: {
        name: Lampa.Lang.translate('lampac_app_settings_load'),
      },
      onChange: function() {
  ...
  if (!window.lampac_app_settings_plugin) startPlugin();
