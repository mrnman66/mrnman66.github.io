(function () {
    ...
    // ============================================================
    // Для input-полей: values[name] = '' (строка), тогда update$3
    // пойдёт по ветке typeof values[name]=='string' → val = key
    ...
    // Для select-полей: values[name] = объект {ключ: отображение}
    ...
    function searchTorrents(movie, callback, error) {
        ...
        Lampa.Network.silent(url, function (data) {
            ...
            var sortField = cfg.sort === 'sid' ? 'Seeders' : cfg.sort === 'date' ? 'PublishDate' :
    ...
    function createCard(item, movie) {
        ...
        card.on('hover:enter', function () {
            ...
            addToTorrs(magnet, (movie.title || '') + ' / ' + (movie.original_title || ''),
    ...
    function init() {
        ...
        // Регистрация компонента-заглушки
        Lampa.Component.add('pidtor', component);

        // Манифест плагина (type: video → появляется в контекстном меню)
        var manifest = {
            type: 'video',
            version: mod_version,
        ...
        // Кнопка на странице карточки (рядом с кнопкой торрентов)
        var buttonHtml =
        ...
        Lampa.Listener.follow('full', function (e) {
            if (e.type === 'complite') {
                ...
                // Вставляем после кнопки торрентов (или после кнопки «Смотреть»)
