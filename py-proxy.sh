...
# Установка зависимостей - отдельная функция
...
[ -z "$LATEST_VER" ] && { 
echo -e "${YELLOW}⚠ Не удалось проверить обновления (сеть/файл недоступен)${NC}\n${CYAN}Продолжить переустановку текущей версии?${NC}"
echo -en "${GREEN}[y/N]: ${NC}"; read confirm
[ "$confirm" != "y" ] && [ "$confirm" != "Y" ] && { PAUSE; return 1; }
}
...
# Проверяем зависимости ПЕРЕД удалением старой версии
echo -e "${CYAN}Проверяем зависимости перед обновлением...${NC}"
install_dependencies || {
echo -e "\n${RED}Ошибка проверки зависимостей! Старая версия сохранена.${NC}"
PAUSE; return 1
}

echo -e "${CYAN}Секрет сохранён — ссылка для подключения не изменится${NC}"

# Теперь можно безопасно удалять старую версию
/etc/init.d/tg-ws-proxy stop >/dev/null 2>&1
/etc/init.d/tg-ws-proxy disable >/dev/null 2>&1
...
# Переустановка с сохранением секрета
reinstall_TG_PH() {
echo -e "\n${MAGENTA}Переустановка TG WS Proxy Python${NC}"

# Сохраняем SECRET перед удалением
OLD_SECRET="$(sed -n 's/.*--secret[[:space:]]*\([0-9a-fA-F]\{32\}\).*/\1/p' "$INIT_PATH_PH" 2>/dev/null)"
if [ -z "$OLD_SECRET" ]; then
echo -e "\n${RED}Не удалось прочитать текущий секрет! Переустановка прервана.${NC}"
PAUSE; return 1
fi

echo -e "${CYAN}Секрет сохранён: ${NC}$OLD_SECRET"
...
echo -e "${CYAN}Старая версия удалена, начинаем чистую установку...${NC}"
...
echo -e "${CYAN}3)${GREEN} Переустановить ${NC}TG WS Proxy Python ${CYAN}(с сохранением секрета)${NC}"
fi
...
3) if [ $IS_INSTALLED -eq 1 ]; then reinstall_TG_PH; else echo -e "\n${RED}Сначала установите прокси!${NC}"; PAUSE; fi;;
*) echo; exit 0 ;;
esac
}
